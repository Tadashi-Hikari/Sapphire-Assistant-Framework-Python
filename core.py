...
def post_office(message): # this is the primary routing service
    ...
    if 'core' in message:
        ...
        if message['payload'] == "No matches" and message['from'] == "parser": 
            ...
            notify("Sending off to the command formatter")
            ...
            user_notify("How did I even end up here?")
            print("Should I just kill it at the prior command?")
...
def start_applications(): # run at the start of core.py
    ...
    # read the config file info for what should and what shouldn't start. generate default config that starts stt, ui, and parser server
...
def install():
    # run the bash script for setting up assistatnt
    # this should run a simple command/utterance builder. Only command is needed when invoked. commands can also be made organically through use
