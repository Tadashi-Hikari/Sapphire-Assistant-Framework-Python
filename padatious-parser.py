...
''' this whole program needs to be reworked, since it currently is assumed that the command alias is going to be called. supposidly, I can set padatious to call the alias, or the command itself. This means tagging the utterance with both, I supposed. I guess command formatter is where I search the database for whcih commad/flag is used '''

def load_config():
    config = configparser.ConfigParser()
    config.read(CONFIG)

    command_table = "commands" # this is gonig to get called twice, it should just get the list of commands from the alias column in commands, and ignore duplicates. It should also probably call from an env variable, since it's now referenced here and padatious
    trianing_utterances_table = "utterances" # this should be renamed padatious_training, not utterances
...
def train(): # I need to find a way to supress the output of this..
    ...
    for record in records: # records is a list, record is a dict. records of commands
        ...
        for i,utterance in enumerate(utterances): # for every utterance in the database...
            if utterance["command"] == record["alias"]: # check if the utterance matches a known command. If this crashes, make sure the csv column headers match
        ...
        container.add_intent(record["alias"], record["utterances"], True)
    # notify("Training")
    container.train()

# This literally does nothing, except show the default example of how padatious works
def example():
    container = IntentContainer('intent_cache')
